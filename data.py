...
# Antibiotics
drug_class1 = DrugClass(name="Antibiotics", user_id=1)

session.add(drug_class1)
session.commit()

# Amoxicillin
drug1 = Drug(name="Amoxicillin", drug_class_name="Antibiotics", user_id=1, information=" ".join([
...
# Metronidazole
drug2 = Drug(name="Metronidazole", drug_class_name="Antibiotics", user_id=1, information=" ".join([
...
# Azithromycin
drug3 = Drug(name="Azithromycin", drug_class_name="Antibiotics", user_id=1, information=" ".join([
...
# Antihypertensives
drug_class2 = DrugClass(name="Antihypertensives", user_id=1)

session.add(drug_class2)
session.commit()

# Atenolol
drug4 = Drug(name="Atenolol", drug_class_name="Antihypertensives", user_id=1, information=" ".join([
...
# Lercanidipine
drug5 = Drug(name="Lercanidipine", drug_class_name="Antihypertensives", user_id=1, information=" ".join([
...
# Perindopril
drug6 = Drug(name="Perindopril", drug_class_name="Antihypertensives", user_id=1, information=" ".join([
...
# Antihistamines
drug_class3 = DrugClass(name="Antihistamines", user_id=1)

session.add(drug_class3)
session.commit()

# Fexofenadine
drug7 = Drug(name="Fexofenadine", drug_class_name="Antihistamines", user_id=1, information=" ".join([
...
# Cyproheptadine
drug8 = Drug(name="Cyproheptadine", drug_class_name="Antihistamines", user_id=1, information=" ".join([
...
# Immunomodulators
drug_class4 = DrugClass(name="Immunomodulators", user_id=1)

session.add(drug_class4)
session.commit()

# Sulfasalazine
drug9 = Drug(name="Sulfasalazine", drug_class_name="Immunomodulators", user_id=1, information=" ".join([
...
# Adalimumab
drug10 = Drug(name="Adalimumab", drug_class_name="Immunomodulators", user_id=1, information=" ".join([
...
drug_class5 = DrugClass(name="Antineoplastics", user_id=1)

session.add(drug_class5)
session.commit()

drug11 = Drug(name="Exemestane", drug_class_name="Antineoplastics", user_id=1, information=" ".join([
                                'Exemestane is an aromatase inhibitor which reduces tissue estrogen concentration. '
                                'It is used to treat hormone receptor positive advanced breast cancer in '
                                'postmenopausal women and in early breast cancer for premenopausal women. '
                                'Common side effects include hot flushes, vaginal dryness, nausea and vomiting, '
                                'hair loss, and peripheral oedema. Exemestane should be taken with food. While on '
                                'this medication, bone density monitoring and calcium and vitamin D supplementation'
                                ' is required. Compared to selective oestrogen receptor modulators, Exemestane causes'
                                ' less endometrial hyperplasia and VTE but more joint pain, muscle pain, bone '
                                'fractures and CV problems.'
                               ]))
session.add(drug11)
session.commit()


drug12 = Drug(name="Goserelin", drug_class_name="Antineoplastics", user_id=1, information=" ".join([
...
# New Drugs
# Umeclidinium
drug13 = NewDrugs(name="Umeclidinium", user_id=1, drug_class="Anticholinergics", information=" ".join([
                                    ...
                                    'FEV1 <50% and recurrent exacerbations. It must be used with caution in people '
                                    'with cardiovascular disorders, glaucoma, and bladder obstruction. It is generally'
                                    ' a well tolerated medication. Common side effects include dry mouth, '
                                    'nasopharyngitis, taste disturbance and throat irritation. It can less frequently '
                                    'cause blurred vision, dizziness,and urinary retention.'
                                    ]))
session.add(drug13)
session.commit()


drug14 = NewDrugs(name="Milnacipran", user_id=1, drug_class="SNRI", information=" ".join([
